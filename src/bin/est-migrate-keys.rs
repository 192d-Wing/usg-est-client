...
fn print_usage(program: &str) {
    ...
    println!();
    println!("EXAMPLES:");
    println!("    {} --key-file key.pem --thumbprint A1B2C3... --label Device", program);
    println!("    {} --key-file key.pem --thumbprint A1B2... --provider \"Microsoft Platform Crypto Provider\"", program);
    println!();
}
...
fn run_migration(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key_file = get_arg(args, "--key-file")
        .ok_or("Missing required argument: --key-file")?;
    let thumbprint = get_arg(args, "--thumbprint")
        .ok_or("Missing required argument: --thumbprint")?;
    let label = get_arg(args, "--label")
        .ok_or("Missing required argument: --label")?;
    let provider_name = get_arg(args, "--provider")
        .unwrap_or(providers::SOFTWARE.to_string());
    let store_path = get_arg(args, "--store")
        .unwrap_or("LocalMachine\\My".to_string());
    ...
    let cert_info = store.find_by_thumbprint(&thumbprint)?
...
#[cfg(windows)]
fn detect_key_algorithm(pem: &str) -> Result<usg_est_client::hsm::KeyAlgorithm, Box<dyn std::error::Error>> {
